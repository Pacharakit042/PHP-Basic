<?php
session_start();
require '../config.php'; // ตรวจสอบสิทธิ์ admin
require 'auth_admin.php'; // ตรวจสอบว่าได้ส่ง id สินค้ามาหรือไม่
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $price > 0) {
        ...
        // 2) ถ้ามีอัปโหลดไฟล์ใหม่ → ตรวจแล้วเซฟไฟล์ และตั้งชื่อใหม่ทับค่า
        if (!empty($_FILES['product_image']['name'])) {
            ...
            if (in_array($file['type'], $allowed, true) && $file['error'] === UPLOAD_ERR_OK) {
                ...
                if (!move_uploaded_file($file['tmp_name'], $destPath)) {
                    // ถ้าย้ายไม่ได ้ อาจตั้ง flash message แล้วคงใช้รูปเดิมไว้
        ...
        // อัปเดต DB
        $sql = "UPDATE products
        SET product_name = ?, description = ?, price = ?, stock = ?, category_id = ?, image = ? WHERE product_id = ?";
        $args = [$name, $description, $price, $stock, $category_id, $newImageName, $product_id];
        $stmt = $conn->prepare($sql);
        $stmt->execute($args);
        // ลบไฟล์เก่าในดิสก์ ถ้า:
        // - มีรูปเดิม ($oldImage) และ
        // - เกิดการเปลี่ยนรูป (อัปโหลดใหม่ หรือ สั่งลบรูปเดิม)
...
<body class="container mt-4">
    ...
    <form method="post" enctype="multipart/form-data" class="row g-3">
        ...
        <!--TODO ===div แสดงรูปเด็ม + เก็บค่าเก่า -->
        <div class="col-md-6">
